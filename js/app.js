...
function barChart(el,{labels,datasets,min,max,yStep=50,horizontal=false,showLegend=false}){
 ...
 datasets.filter(d=>d.kind==="line").forEach(ds=>{const y=scaleY(ds.value);svg.appendChild(svgEl("line",{x1:m.l,y1:y,x2:w-m.r,y2:y,stroke:ds.color,"stroke-width":"2","stroke-dasharray":"8 6"}));let tx=svgEl("text",{x:w-m.r-2,y:y-5,"text-anchor":"end","font-size":"10","font-weight":"700",fill:ds.color});tx.textContent=ds.label;svg.appendChild(tx)});
...
function lineChart(el,{labels,datasets,min,max,step=10}){
 clear(el);const{w,h}=fit(el);const svg=svgEl("svg",{viewBox:`0 0 ${w} ${h}`});el.appendChild(svg);const m={l:55,r:25,t:28,b:42},pw=w-m.l-m.r,ph=h-m.t-m.b;const sx=i=>m.l+(labels.length===1?pw/2:i*pw/(labels.length-1));const sy=v=>m.t+ph-(v-min)/(max-min)*ph;
 for(let v=min;v<=max;v+=step){const y=sy(v);svg.appendChild(svgEl("line",{x1:m.l,y1:y,x2:w-m.r,y2:y,stroke:"#e8edf3"}));let t=svgEl("text",{x:m.l-10,y:y+4,"text-anchor":"end","font-size":"10",fill:"#596170"});t.textContent=v;svg.appendChild(t)}
 datasets.forEach((ds,di)=>{const pts=ds.values.map((v,i)=>`${sx(i)},${sy(v)}`).join(" ");svg.appendChild(svgEl("polyline",{points:pts,fill:"none",stroke:ds.color,"stroke-width":"3"}));ds.values.forEach((v,i)=>{svg.appendChild(svgEl("circle",{cx:sx(i),cy:sy(v),r:4,fill:ds.color}));let t=svgEl("text",{x:sx(i)+(di===0?0:di===1?-6:7),y:sy(v)+(di===0?16:di===1?-10:-24),"text-anchor":"middle","font-size":"10","font-weight":"700",fill:ds.color});t.textContent=Math.round(v);svg.appendChild(t)})});
 labels.forEach((l,i)=>{let t=svgEl("text",{x:sx(i),y:h-16,"text-anchor":"middle","font-size":"10",fill:"#596170"});t.textContent=l;svg.appendChild(t)});
}
...
function renderLevels(){
 const a=levelArea.value;const rows=D.levels[a].filter(r=>r.year===2025);
...
function renderGaps(){
 ...
 const el=document.getElementById("gapChart");clear(el);const{w,h}=fit(el);const svg=svgEl("svg",{viewBox:`0 0 ${w} ${h}`});el.appendChild(svg);const m={l:55,r:25,t:25,b:50},pw=w-m.l-m.r,ph=h-m.t-m.b,min=320,max=470,sy=v=>m.t+ph-(v-min)/(max-min)*ph,g=pw/labels.length,bw=Math.min(52,(g-22)/2);
 for(let v=320;v<=470;v+=25){const y=sy(v);svg.appendChild(svgEl("line",{x1:m.l,y1:y,x2:w-m.r,y2:y,stroke:"#e8edf3"}));let t=svgEl("text",{x:m.l-10,y:y+4,"text-anchor":"end","font-size":"10",fill:"#596170"});t.textContent=v;svg.appendChild(t)}
 labels.forEach((lab,i)=>{const cx=m.l+g*i+g/2;["2022","2025"].forEach((yr,j)=>{const v=obj[lab][j],x=cx-(bw*2+5)/2+j*(bw+5),y=sy(v),col=j===0?"#cbd2df":C[a];svg.appendChild(svgEl("rect",{x,y,width:bw,height:sy(min)-y,fill:col}));let tx=svgEl("text",{x:x+bw/2,y:y-5,"text-anchor":"middle","font-size":"10","font-weight":"700",fill:j===0?"#687181":C[a]});tx.textContent=v;svg.appendChild(tx)});let t=svgEl("text",{x:cx,y:h-18,"text-anchor":"middle","font-size":"10",fill:"#596170"});t.textContent=lab;svg.appendChild(t)});
}
  ...
  regionalChart.innerHTML=`
    ...
    ${rows.map(([pais,puntaje])=>{
      ...
      return `<div class="regional-row${cls}">
        <div class="regional-country">${pais}</div>
...
window.addEventListener("resize",()=>{clearTimeout(window.__r);window.__r=setTimeout(renderVisible,120)});
renderSummary();
